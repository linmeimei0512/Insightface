...
class CallBackModelCheckpoint(object):
    ...
    def __call__(self, global_step, epoch: int, backbone: torch.nn.Module, partial_fc: PartialFC = None):
        if global_step > 100 and self.rank is 0:
            # torch.save(backbone.module.state_dict(), os.path.join(self.output, "backbone.pth"))
            torch.save(backbone.module.state_dict(), os.path.join(self.output, str(epoch), "_weight_backbone.pth"))
            torch.save(backbone.module, os.path.join(self.output, str(epoch), "_backbone.pth"))
